...
void DEInteg(Matrix& f (double t, Matrix &y), double &t, double tout, double relerr,
  double abserr, int n_eqn, Matrix &y, int &iflag, double work[], int iwork[]) {
	ode(f, n_eqn, *y.data, t, tout, relerr, abserr, iflag, work, iwork);
}
...
//
  ...
  if ( iflag != 1 )
  {
    if ( t != told )
    {
      ...
      cout << "\n";
      cout << "DE - Fatal error!\n";
      cout << "  IFLAG is not 1, and T is not equal to TOLD.\n";
      exit ( 1 );
    }
...
//
    ...
    {
      h = tout - x;
	  Matrix &YY = zeros(neqn); *YY.data = yy;
      Matrix &YP = f ( x, YY );
	  yp = *YP.data;
      for ( l = 1; l <= neqn; l++ )
      {
