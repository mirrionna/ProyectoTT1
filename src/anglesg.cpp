...
tuple<Matrix&,Matrix&> anglesg (double az1,double az2,double az3,double el1,double el2,double el3,double Mjd1,double Mjd2,double Mjd3,Matrix& Rs1,Matrix& Rs2,Matrix& Rs3){
	...
	auto [lon1, lat1, h1] = Geodetic(transpose(Rs1));
	auto [lon2, lat2, h2] = Geodetic(transpose(Rs2));
	auto [lon3, lat3, h3] = Geodetic(transpose(Rs3));
	...
	double a1 = tau3/(tau3-tau1)*1.0;
	double a3 =-tau1/(tau3-tau1)*1.0;
	...
	coef[0]=  1.0;  // R2^8... polynomial
	coef[1]=  0.0;
	coef[2]=  -(pow(d1s,2) + d1s*Ccye + pow(norm(transpose(Rs2)),2));
	coef[3]=  0.0;
	coef[4]=  0.0;
	coef[5]=  -SAT_Const::GM_Earth*(d2s*Ccye + 2*d1s*d2s);
	coef[6]=  0.0;
	coef[7]=  0.0;
	coef[8]=  -pow(SAT_Const::GM_Earth,2)*pow(d2s,2);
	int nr = real_poly_roots(coef, 8, zeror, zeroi);
	...
	while ((abs(rhoold2-rho2) > 1e-12) && (ll <= 0 )){
		...
		auto [v2gibbs, theta,theta1,copa,error] = gibbs(transpose(r1),transpose(r2),transpose(r3));
		v2 = v2gibbs;
		if ( (error.compare("ok")!=0) && (copa < std::numbers::pi/180) ){ 
			auto [v2hgibbs,theta,theta1,copa,error] = hgibbs(transpose(r1),transpose(r2),transpose(r3),Mjd1,Mjd2,Mjd3);
			v2 = v2hgibbs;
		}
		Matrix& auxelements = union_vector(transpose(r2),v2);
		auto [p, a, e, i, Omega, omega, M] = elements (auxelements);	
		double f1,g1,f3,g3;
		if ( ll <= 8 ){
	...
	return std::tie(r2,transpose(v2));
}